...
Alpine.data("mapDisplay", (data) => ({
    ...
    init(){
        ...
        const map = L.map(mapElement).setView([this.location.lat, this.location.lng], 16);
        ...
        const marker = L.marker([this.location.lat, this.location.lng]).addTo(map);
...
Alpine.data('evaluationFormGenerator', () => ({
    ...
    removeField(localId){
        this.form = {
            ...this.form,
            fields : this.form.fields.filter((field) => field.localId !== localId)
        }
    }
}));
